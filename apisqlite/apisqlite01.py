    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            2) Search for All Movies Containing Type
            3) Search for All Movies Containing Year
            99) Exit""")
        ...
        if answer == "2":
            searchtype = input("Search all movies in the OMDB. Enter search type: ")
            resp = movielookup(mykey, searchtype)["Type"]
            print(resp)
            trackmeplease(resp)

        if answer == "3":
            searchyear = input("Search all movies in the OMDB. Enter release year: ")
            resp = movielookup(mykey, searchyear)["Year"]
            print(resp)
            trackmeplease(resp)
